...
var app = express();
var url='mongodb+srv://ICP6:mongo@cluster0-fe1po.mongodb.net/Library';
//Add your cluster URL till /test...here Library is my db name and Bookdetails is my table name
...
app.get('/get', function (req, res) {
    MongoClient.connect(url, function(err, db) {
        ...
        dbase.collection('Bookdetails').find().toArray(function(err, result){
            if(err)
            {
...
app.get('/delete/:toBeDeleted_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        dbase.collection('Bookdetails').deleteOne({_id: ObjectID(req.params.toBeDeleted_id)}, function(err, result){
            if(err)
            {
...
app.post('/update/:toBeUpdated_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        dbase.collection("Bookdetails").updateMany(myquery, newvalues, function (err, res) {
            if (err) throw err;
...
var insertDocument = function(db, data, callback) {
    db.collection('Bookdetails').insertOne( data, function(err, result) {
        if(err)
        {
